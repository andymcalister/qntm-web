...
// Range windows are calendar-day based, so 1W spans the weekend (7 calendar days
// back from the latest session, not 7 trading days).
...
export default function EquityChart({ curve }: { curve: CurvePoint[] }) {
  ...
  // which range buttons have at least 2 sessions to show
  const enabled = useMemo(() => {
    const set = new Set<string>();
    for (const r of RANGES) {
      if (r.days == null) { set.add(r.key); continue; }
  ...
  const geom = useMemo(() => {
    ...
    const ticks = Array.from(new Set([0, Math.floor(n / 2), n - 1])).map((i) => ({
      x: X(i), label: data[i].d.slice(5).replace("-", "/"),
