...
// Server-side paywall lives on the API — this just forwards the caller's token so
// the API can read their plan and decide full gems vs. locked teaser.
export async function GET(req: NextRequest) {
  const t = token(req);
  if (!t) return NextResponse.json({ error: "unauthenticated" }, { status: 401 });
  try {
    const res = await fetch(`${API_BASE}/api/hidden-gems`, {
