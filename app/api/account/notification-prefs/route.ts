...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/account/notification-prefs`, { headers: { Authorization: `Bearer ${t}` }, cache: "no-store" });
...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/account/notification-prefs`, {
