...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/alerts`, { headers: { Authorization: `Bearer ${t}` }, cache: "no-store" });
...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/alerts`, {
      ...
      body: JSON.stringify({ ticker: body?.ticker ?? null, kind: body?.kind, threshold: body?.threshold ?? null, scope: body?.scope || "ticker" }),
      cache: "no-store",
    });
    return NextResponse.json(await res.json().catch(() => ({})), { status: res.status });
  } catch { return NextResponse.json({ error: "upstream_unreachable" }, { status: 502 }); }
